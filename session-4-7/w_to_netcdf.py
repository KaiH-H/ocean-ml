...
for d in range(0,dates): # loops through number of dates
	...
	w_file.readline() # skips the header in the w file
	w_file.readline()
...
longitude[:] = longitude_val # loops through longitude 
latitude[:] = latitude_val # loops through latitude 
time[:] = time_val
depth_arr = (-1)*np.arange(z_step,(levels+1)*z_step,z_step)  # loops through depth 
depth[:]= depth_arr

w[:] = w_array[:,2:78,2:25,:] # takes values from w file and puts it into numpy array
